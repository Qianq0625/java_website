...
protected void service(HttpServletRequest req, HttpServletResponse resp)
		...
		EASYBUY_ORDER order=new EASYBUY_ORDER(Integer.parseInt(orderId), null, name, addres, null, Integer.parseInt(cost), Integer.parseInt(tatus), 1);
		int count=EASYBUY_ORDERDao.update(order);
		if(count>0){
			resp.sendRedirect("manage-result.jsp");  //Ìø×ª
		}else{
